---
name: anthropic-extractor
tier: 2
last_reviewed: 2026-04-19
---

# anthropic-extractor

## Compiled truth (as of 2026-04-19)

_Draft seed — Edward to refine._

- Direct-Anthropic extraction path vs OpenAI proxy. Cost-reduction project.
- Mention count in agent-logs: **5** (across 2026-04-03 → 2026-04-13)
- Role / relationship: _TBD_
- Current focus: _TBD_
- Working style: _TBD_
- Risks / watch-outs: _TBD_

## Timeline

- 2026-04-03 — Anthropic Extractor — Research & Scaffold Session — `agent-logs/2026-04-03_anthropic-extractor-research-and-scaffold.md`
- 2026-04-03 — Created `~/Documents/rm-ops/truthly/anthropic-extractor/` with a full CLAUDE.md brief for building a Python CLI tool to pull Anthropic admin usage data into Duc… — `agent-logs/2026-04-03_truthly-anthropic-cost-reduction.md`
- 2026-04-04 — Anthropic Extractor — Mozart Integration Session — `agent-logs/2026-04-04_anthropic-extractor-mozart-integration.md`
- 2026-04-05 — Instructions at `~/Documents/rm-ops/truthly/anthropic-extractor/MOZART_INTEGRATION.md` — `agent-logs/2026-04-05_tap-anthropic-admin-repo-setup.md`
- 2026-04-13 — `truthly/anthropic-extractor/` — dlt-based extractor, local copy. — `agent-logs/2026-04-13_truthly-mixpanel-cost-observability.md`
- 2026-04-19 — Concepts: TRUTHLYDEV-board 103 · mozart-promotion-pipeline 22 · anthropic-extractor 5 · worktree-cleanup 2 — `agent-logs/2026-04-19-rm-skills-repo-and-brain-backfill.md`
